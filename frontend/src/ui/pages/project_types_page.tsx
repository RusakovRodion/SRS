            ...
            <div className="list">
                {projectTypesList.map((projectType) => (
                    <div key={projectType.id} className={list_item}>
                        ...
                        <ListTools
                            onAdd={null}
                            onView={() => onView(projectType.id)}
                            onEdit={() => handleOpenForm(projectType)}
                            onDelete={() => onDelete(projectType.id)}
                        />
                    </div>
                ))}
            </div>
...
export const AddPtForm: React.FC<AddPtFormProps> = ({
    ...
    const [formState, setFormState] = useState<AddPtFormData>(InitialData);
