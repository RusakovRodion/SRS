...
export function MainPage<T>(title: string, ref:string, data:T[],  values:string[]) {
           ...
           <ListComponent<T>
                ...
                renderOption={(item) => (
                    <div className={list_item}>
                        {
                            values.map(a => <div className={list_item_info}>{item[a]}</div>)
                        }
                        <tools>
                            <ChangesButtonOnlyIcon onClick={() => console.log("changes button")} />
                            <WatchButtonOnlyIcon onClick={() => navigate("/" + ref + "/" + item.id)} />
